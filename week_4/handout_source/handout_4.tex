...
\end{equation}
%
Finally, by replacing elements of Equation~\ref{equ:trig} with those from Equation~\ref{equ:xy} we may obtain straight forward transformations to lead from $(x, y)$ to $(x', y')$, 
%
\begin{equation}
  \begin{aligned}
...
Using the transformations outlined in Equation~\ref{equ:final}, you now must create module named \texttt{transform} that includes a function called \texttt{rotation} that will take three variables \texttt{x}, \texttt{y}, and \texttt{angle}.
